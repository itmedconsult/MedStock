...
export function StockOperationWorkspace({ operation }: { operation: Operation }) {
  ...
  const queueItem = (item: InventoryItem, useDefaults = false) => {
    const next: QueueItem = {
      ...
      typeBefore: item.containerType, quantityBefore: item.quantity, unit: item.unit, trackMode: item.trackMode, statusBefore: item.status,
      note: useDefaults ? "" : note, actualType: useDefaults ? item.containerType as StockType : actualType,
      actualQuantity: useDefaults ? item.quantity : actualQuantity, cutReason, cutMode: item.trackMode === "UNIT" ? "ALL" : cutMode,
      cutQuantity: item.trackMode === "UNIT" || cutMode === "ALL" ? item.quantity : cutQuantity,
    };
  ...
  const calculatedInput = scanned ? (isCheck ? checkResult({ id: scanned.id, barcode: scanned.id, sku: scanned.sku, productName: scanned.productName, branch: scanned.location, typeBefore: scanned.containerType, quantityBefore: scanned.quantity, unit: scanned.unit, trackMode: scanned.trackMode, statusBefore: scanned.status, note, actualType, actualQuantity, cutReason, cutMode, cutQuantity }) : cutResult({ id: scanned.id, barcode: scanned.id, sku: scanned.sku, productName: scanned.productName, branch: scanned.location, typeBefore: scanned.containerType, quantityBefore: scanned.quantity, unit: scanned.unit, trackMode: scanned.trackMode, statusBefore: scanned.status, note, actualType, actualQuantity, cutReason, cutMode, cutQuantity })) : null;
      ...
      <section className={styles.workspace}>
        <div className={styles.scannerPanel}>
          ...
          <div className={styles.formGrid}>
            ...
            <label><span>Current stock type</span><input value={scanned?.containerType ?? ""} placeholder="—" readOnly /></label><label><span>Current quantity</span><input value={scanned?.quantity ?? ""} placeholder="—" readOnly /></label>
            ...
            {isCheck ? <><label className={styles.fullField}><span>Reason / note</span><input value={note} onChange={(event) => setNote(event.target.value)} placeholder="Required when count or type changes" disabled={!scanned} /></label><label><span>Actual stock type</span><select value={actualType} onChange={(event) => setActualType(event.target.value as StockType)} disabled={!scanned}><option>FULL</option><option>OPEN</option></select></label><label><span>Actual quantity found</span><input type="number" min="0" step="0.000001" value={actualQuantity} onChange={(event) => setActualQuantity(Number(event.target.value))} disabled={!scanned} /></label><label><span>Quantity difference</span><input value={calculatedInput && "difference" in calculatedInput ? calculatedInput.difference : ""} readOnly /></label><label><span>Status after check</span><input value={calculatedInput?.statusAfter ?? ""} readOnly /></label></> : <><label><span>Reason</span><select value={cutReason} onChange={(event) => setCutReason(event.target.value as CutReason)} disabled={!scanned}><option>SALE</option><option>USE</option></select></label><label><span>Cut mode</span><select value={cutMode} onChange={(event) => { const mode = event.target.value as CutMode; setCutMode(mode); if (mode === "ALL" && scanned) setCutQuantity(scanned.quantity); }} disabled={!scanned || scanned.trackMode === "UNIT"}><option>ALL</option><option>PARTIAL</option></select></label><label><span>Quantity to cut</span><input type="number" min="0.000001" step="0.000001" value={cutMode === "ALL" && scanned ? scanned.quantity : cutQuantity} onChange={(event) => setCutQuantity(Number(event.target.value))} disabled={!scanned || cutMode === "ALL"} /></label><label><span>Quantity after cut</span><input value={calculatedInput?.quantityAfter ?? ""} readOnly /></label><label className={styles.fullField}><span>Stock type after cut</span><input value={calculatedInput?.typeAfter ?? ""} readOnly /></label></>}
          </div>

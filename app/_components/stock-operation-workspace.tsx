...
function checkResult(item: QueueItem) {
  ...
  if (isPacked(item) && ((isDiscreteUnit(item.unit) && !Number.isInteger(quantity)) || quantity > Number(item.unitsPerPack))) errors.push("INVALID CONTAINER CONTENT COUNT");
  if (isPacked(item) && quantity > 0 && item.actualType === "FULL" && quantity !== item.unitsPerPack) errors.push("PARTIAL CONTAINER MUST BE OPEN");
